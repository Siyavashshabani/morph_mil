...
class TensorAugment(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        dev = x.device
        # TokenDrop
        if torch.rand((), device=dev) < self.p_token_drop:
            k = max(1, int(T * self.token_drop_frac))
            for b in range(B):
                idx = torch.randperm(T, device=dev)[:k]
                if self.token_drop_mode == "zero":
                    x[b, idx] = 0
                else:
                    x[b, idx] = torch.randn_like(x[b, idx]) * self.feat_jitter_sigma
        # TokenShuffle
        if torch.rand((), device=dev) < self.p_token_shuffle:
            if self.local_shuffle and self.local_window > 1:
                ...
                x = x[:, perm]
        # Token span mask
        if torch.rand((), device=dev) < self.p_token_span_mask:
            span = max(1, int(T * self.token_span_frac))
            for b in range(B):
                s = torch.randint(0, T - span + 1, (1,), device=dev).item()
                x[b, s:s+span] = 0
        # Feature jitter
        if torch.rand((), device=dev) < self.p_feat_jitter:
            x = x + torch.randn_like(x) * self.feat_jitter_sigma
        # Feature band mask
        if torch.rand((), device=dev) < self.p_feat_band_mask:
            bw = max(1, int(D * self.feat_band_frac))
            for b in range(B):
                s = torch.randint(0, D - bw + 1, (1,), device=dev).item()
                x[b, :, s:s+bw] = 0
        ...
        if torch.rand((), device=dev) < self.p_feat_affine:
            ...
            x = gamma * x + beta
        # Rect cutout
        if torch.rand((), device=dev) < self.p_rect_cutout:
            th = max(1, int(T * self.rect_token_frac))
            dw = max(1, int(D * self.rect_feat_frac))
            for b in range(B):
                ts = torch.randint(0, T - th + 1, (1,), device=dev).item()
                ds = torch.randint(0, D - dw + 1, (1,), device=dev).item()
                x[b, ts:ts+th, ds:ds+dw] = 0
