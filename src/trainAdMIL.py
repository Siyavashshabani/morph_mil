...
from utils import build_attn_maps, load_batch_images, save_overlays
import shutil
...
from model.adMIL import GATE_AB_MIL
...
def build_backbone(cfg):
    ...
    elif backbone == "poolMIL":
        device = torch.device(f"cuda:{cfg['cuda']}" if torch.cuda.is_available() else "cpu")
        model = PoolMIL(cfg= cfg, n_classes = n_classes).to(device)
        return model

    elif backbone == "adMIL":
        if cfg["base_bakcbone"]=="resnet": 
            input_dim = 2048
        elif cfg["base_bakcbone"]=="dinov3":   
            input_dim = 1024    
                
        device = torch.device(f"cuda:{cfg['cuda']}" if torch.cuda.is_available() else "cpu")
        model = GATE_AB_MIL(L=1024, D=128, num_classes=n_classes,in_dim=input_dim ).to(device) #
        print("falg-------------------------------")
        return model         #L=1024, D=128 
...
class Trainer:
    ...
    def _unpack(self, out):
        logits   = out["logits"]
        wsi_attn = out["WSI_attn"] 
        wsi_feat = out["WSI_feature"]
        return logits, wsi_feat, wsi_attn
    ...
    @torch.no_grad()
    def _visual_att_on_loader(self, loader):
                
        out_dir = Path("pics/overlays")
        if out_dir.exists():
            shutil.rmtree(out_dir)        # remove the whole folder and its contents
        out_dir.mkdir(parents=True, exist_ok=True)  # recreate it empty        
        ...
        for batch in loader:
            ...
            logits, WSI_feature, WSI_attn = self._unpack(out)
            # print("WSI_attn -------------------", WSI_attn.shape)
            # print("WSI_attn -------------------", WSI_attn[0])
            ...
            ##---------------------------------------read the image--------------------------------------- 
            paths = batch["path"]           # str | Path | list
            imgs_BCHW = load_batch_images(paths, device=self.device)
            print("imgs_BCHW shape:", imgs_BCHW.shape)  # -> (B, 3, 1024, 1024)
            
            ##-----------------------------------create the attention map--------------------------------- 
            attn_map, attn_norm, attn_ups = build_attn_maps(WSI_attn, 
                                                            out_hw=(1024,1024), 
                                                            mode=self.cfg["att_map_mode"]) 
            print("attn_ups shape:", attn_ups.shape )  # -> (B, 3, 1024, 1024)

            ##-----------------------------------plotting the overlays--------------------------------- 
            save_overlays(imgs_BCHW, attn_ups, 
                        out_dir=out_dir,
                        paths=batch["path"], 
                        labels=batch["label"],
                        preds = preds,
    ...
    def test(self, loader=None, name="test"):
        ...
        ### plot the visual attention scores
        if self.cfg["att_vis"]== True: 
            self._visual_att_on_loader(loader)
        ...
        if self.cfg["backbone"] in ("TransMIL", "adMIL"):
    ...
    def train_one_epoch(self, epoch: int):
        ...
        num_batches = len(self.train_loader)   # number of batches per epoch
...
# ---------- Main ----------
def main():
    cfg_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "src/trainAdMIL.yaml"))
