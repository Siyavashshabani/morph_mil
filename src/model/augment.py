...
class TensorAugment(nn.Module):
    ...
    def _token_mix(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # IMPORTANT: sample -> float32, then cast to x dtype/device
        lam = beta.sample((B, 1, 1)).to(device=dev, dtype=dtype)  # [B,1,1] same dtype as x

        one = torch.ones((), device=dev, dtype=dtype)  # scalar 1.0 in same dtype

        for b in range(B):
            idx = torch.randperm(T, device=dev)[:k]
            jdx = torch.randperm(T, device=dev)[:k]
            x[b, idx] = lam[b] * x[b, idx] + (one - lam[b]) * x[b, jdx]
