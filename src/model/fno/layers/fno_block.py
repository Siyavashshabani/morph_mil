...
class FNOBlocks(nn.Module):
    ...
    def forward(self, x, index=0, output_shape=None):
        if self.preactivation:
            # print("Warning: FNOBlock is set to preactivation mode")
            return self.forward_with_preactivation(x, index, output_shape)
        else:
            # print("Warning: FNOBlock is set to postactivation mode")
            return self.forward_with_postactivation(x, index, output_shape)

    def forward_with_postactivation(self, x, index=0, output_shape=None):
        # print("Start forward_with_postactivation--------------------------------------")
        x_skip_fno = self.fno_skips[index](x)
        # print("After fno_skip--------------------------------------",x_skip_fno.shape )
        
        x_skip_fno = self.convs[index].transform(x_skip_fno, output_shape=output_shape)

        if self.use_channel_mlp:  
            # print("use_channel_mlp is True, applying channel_mlp_skips")
            x_skip_channel_mlp = self.channel_mlp_skips[index](x)
            x_skip_channel_mlp = self.convs[index].transform(x_skip_channel_mlp, output_shape=output_shape)
            # print("x_skip_channel_mlp---------------------------------------", x_skip_channel_mlp.shape)
